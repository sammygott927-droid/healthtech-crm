 ...
 *
 * Demo-only: composes the SAME email digest body the cron would have
 * sent, but returns it as JSON ({ html, subject, date }) instead of
 * shipping it through Resend.
 *
 * Reads from today's already-stored daily_briefs rows — does not run
 * the brief pipeline, does not call Claude, does not send any email.
 */
export async function GET(request: NextRequest) {
  ...
  // Brief items — same filter the in-app tab uses.
